      ...
      <Listbox.Options data-cy="sort-parent" className="absolute top-full mt-2 bg-white grid py-2 w-56 rounded-md shadow-lg">
        {sorts.map(sort => (
          <Listbox.Option 
            ...
            value={sort.name}
          >
            <div className="px-7 py-3.5 hover:bg-primary/10 cursor-pointer" data-cy="sort-selection" >
              <div className="flex items-center gap-3">
                <i data-cy="sort-selection-icon">{sort.icon}</i>
                <p className="flex-grow" data-cy="sort-selection-title">{sort.name}</p>
                {sort.name === selected && <div className="self" data-cy="sort-selection-selected">
                  <svg width="18" height="18" viewBox="0 0 18 18" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.75 9L7.5 12.75L15 5.25" stroke="#4A4A4A" strokeLinecap="square"/></svg>
                </div>}
              </div>
            </div>
          </Listbox.Option>
